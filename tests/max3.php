...
if ($argv[1] == 'dump') {
    $sp = websocket_open('127.0.0.1', 2000, '', $error_string, 99999);// max 243 connections actives ?
    $x = websocket_read($sp, $error_string);echo'.';//echo $x."\n";//{"pid":45205,"id":303} served by, fileDescriptor
    websocket_write($sp, '{"dump":1}');//$x = websocket_read($sp, $error_string);
...
if ('Scenario 1 : publishes, then consumes') {
    ...
    try {
        ...
        while (!$connected) {//  php tests/max2.php 2000 1 1
            ...
            if (!$x) {
                if ($log) file_put_contents('8.log', "\nC:" . getMyPid() . "errCode:" . $sp->errCode, 8);//   54:: 2928 connections shitted
...
//46ms pour 7 opérations
        ...
        $discussion = ['{"push":"' . $pushes . '","message":"' . $pushes . 'MessagContains:' . getmyPid() . '-' . uniqid() . '"}',
            '{"status":"free"}',    // says he's free
            '{"suscribe":"' . $receives . '"}',  // then subscribes to the message
        ...
        foreach ($discussion as $q) {
            ...
            $sucess = websocket_read($sp, $error_string);//$cli->recv();//  Last Waits here for something to show up
            if (!$sucess) {
                if ($log) file_put_contents('8.log', "\nC:" . getMyPid() . "errCode:" . $sp->errCode, 8);//8504
                $r->incr('txerr');
                continue;
            } else $x = trim($sucess);
